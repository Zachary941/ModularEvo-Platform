...
def get_graph():
    ...
    nodes = [
        GraphNode(id="codebert", name="CodeBERT", type="pretrained",
                  meta={"params": "124.6M", "arch": "RobertaModel (Encoder-only)",
                        "vocab": "50,265 tokens (BPE)", "hidden": "768 维 × 12 层 × 12 头",
                        "pretrain_data": "CodeSearchNet (6 语言, 6.4M 函数)",
                        "pretrain_task": "MLM + RTD (Replaced Token Detection)",
                        "source": "microsoft/codebert-base",
                        "desc": "微软发布的代码预训练语言模型，支持 NL-PL 跨模态理解"}),
        GraphNode(id="module-java", name="Module-Java", type="module",
                  meta={"params": "124.6M (稀疏)", "wrr": "22.94%", "sparsity": "77.06%",
                        "train_data": "CodeSearchNet-Java (164K 函数)",
                        "train_epochs": "10 epochs", "lr": "5e-5",
                        "desc": "从 CodeBERT 中提取的 Java 语言知识模块，保留 22.94% 关键权重"}),
        GraphNode(id="module-python", name="Module-Python", type="module",
                  meta={"params": "124.6M (稀疏)", "wrr": "24.15%", "sparsity": "75.85%",
                        "train_data": "CodeSearchNet-Python (251K 函数)",
                        "train_epochs": "10 epochs", "lr": "5e-5",
                        "desc": "从 CodeBERT 中提取的 Python 语言知识模块，保留 24.15% 关键权重"}),
        GraphNode(id="ft-clone", name="FT-CloneDet", type="finetuned",
                  meta={"task": "克隆检测 (Clone Detection)", "params": "125.8M",
                        "eval_metric": "F1 = 94.55%",
                        "base_module": "Module-Java",
                        "train_epochs": "5 epochs", "lr": "2e-5", "batch_size": "32",
                        "desc": "基于 Java 模块稀疏微调的代码克隆检测模型，判断两段代码是否语义等价"}),
        GraphNode(id="ft-search", name="FT-CodeSearch", type="finetuned",
                  meta={"task": "代码搜索 (Code Search)", "params": "127.0M",
                        "eval_metric": "F1 = 63.20%, ACC = 55.46%",
                        "base_module": "Module-Python",
                        "train_epochs": "5 epochs", "lr": "2e-5", "batch_size": "32",
                        "desc": "基于 Python 模块稀疏微调的代码搜索模型，将自然语言查询匹配到代码片段"}),
        GraphNode(id="merged", name="Merged-Model", type="merged",
                  meta={"params": "124.6M",
                        "source_models": "FT-CloneDet + FT-CodeSearch",
                        "formula": "θ_merged = θ_base + Σ λᵢ·τᵢ",
                        "desc": "将克隆检测和代码搜索两个微调模型通过任务向量方法合并，实现多任务统一推理"}),
    ]
    edges = [
