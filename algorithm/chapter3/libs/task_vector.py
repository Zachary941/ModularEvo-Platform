...
from .merge_utils import get_param_names_to_merge
...
class TaskVector:
    ...
    def __mul__(self, scalar: float):
        if not isinstance(scalar, (int, float)):
            raise TypeError(f"not support: {type(scalar)}")
            
        new_dict = {}
        for key, tensor in self.task_vector_param_dict.items():
            new_dict[key] = tensor * scalar
    ...
    def combine_with_pretrained_model(self, pretrained_model: nn.Module,scaling_coefficient:float):
        """
        ...
        with torch.no_grad():
            ...
            for param_name in self.task_vector_param_dict:
                ...
                # merged_params[param_name] = pretrained_param_dict[param_name] + self.task_vector_param_dict[param_name]
