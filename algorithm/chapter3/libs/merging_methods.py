...
from .merge_utils import get_param_names_to_merge, get_modules_to_merge
...
class MergingMethod:
    ...
    def task_arithmetic(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list, scaling_coefficients: list = None):
        """
        task arithmetic method
        :param merged_model: nn.Module, the merged model
        :param models_to_merge: list, individual models that need to be merged
        :param exclude_param_names_regex: list, regular expression of names of parameters that need to be excluded
        :param scaling_coefficients: list, scaling coefficients for each task vector
        :return:
        """
        if scaling_coefficients is None:
            scaling_coefficients = [1.0] * len(models_to_merge)
        
        assert len(scaling_coefficients) == len(models_to_merge), "Number of scaling coefficients must match number of models"
        assert all(isinstance(x, float) for x in scaling_coefficients), "All scaling coefficients must be float"

        models_to_merge_task_vectors = [TaskVector(pretrained_model=merged_model, 
        ...
        # iterate each individual model that needs to be merged
        with torch.no_grad():
            scaled_vectors = [vector * coef for vector, coef in zip(models_to_merge_task_vectors, scaling_coefficients)]
            merged_task_vector = scaled_vectors[0]
            for vector in scaled_vectors[1:]:
                merged_task_vector = merged_task_vector + vector

            # combine with parameters of the merged model
    ...
    def ties_merging(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list, param_value_mask_rate: float = 0.8, scaling_coefficient: float = 1.0, scaling_coefficients: list = None):
        """
        ties merging method
        :param merged_model: nn.Module, the merged model
        :param models_to_merge: list, individual models that need to be merged
        ...
        def mask_smallest_magnitude_param_values(flattened_models_to_merge_param: torch.Tensor, param_value_mask_rate: float = 0.8):
            ...
            print(f"num_mask_params: {num_mask_params}, param_value_mask_rate: {param_value_mask_rate}, total_params: {flattened_models_to_merge_param.shape[1]}")
        ...
        with torch.no_grad():
            ...
            scaling_tensor = torch.tensor(scaling_coefficients, device=flattened_models_to_merge_param.device)
            flattened_models_to_merge_param = flattened_models_to_merge_param * scaling_tensor.view(-1, 1)
    ...
    def merging_models(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list, trainers: list = None, scaling_coefficient: float = 1.0,scaling_coefficients: list = None,
        ...
        if self.merging_method_name == "average_merging":
            ...
            merged_params = self.ties_merging(merged_model=merged_model, models_to_merge=models_to_merge, exclude_param_names_regex=exclude_param_names_regex,
                                              param_value_mask_rate=param_value_mask_rate, scaling_coefficient=scaling_coefficient, scaling_coefficients=scaling_coefficients)
    ...
    def get_merged_model(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list, trainers: list = None, scaling_coefficient: float = 1.0,scaling_coefficients: list = None,
                         nums_fisher_examples: list = None, fisher_scaling_coefficients: list = None, normalize_fisher_weight: bool = True, minimal_fisher_weight: float = 1e-6,
                         nums_regmean_examples: list = None, reduce_non_diagonal_ratio: float = 1.0, param_value_mask_rate: float = 0.8,
                         weight_format: str = "delta_weight", weight_mask_rates: list = None, use_weight_rescale: bool = True, mask_strategy: str = "random",
                         mask_apply_method: str = "average_merging", models_use_deepcopy: bool = False):
        ...
        merged_params = self.merging_models(merged_model=merged_model, models_to_merge=models_to_merge, exclude_param_names_regex=exclude_param_names_regex, trainers=trainers,
                                            nums_fisher_examples=nums_fisher_examples, scaling_coefficient=scaling_coefficient, scaling_coefficients=scaling_coefficients, fisher_scaling_coefficients=fisher_scaling_coefficients,
                                            normalize_fisher_weight=normalize_fisher_weight, minimal_fisher_weight=minimal_fisher_weight,
